...
enum BC_OP {
	...
	BC_OP_ISTYPE = 24, // 类型检查
	BC_OP_ISNUM = 25, // 数字检查
	BC_OP_NOT = 26, // A<DST> = not D<VAR>
	BC_OP_MOV = 27, // A<DST> = D<VAR>
	BC_OP_LEN = 29, // A<DST> = #D<VAR>
	BC_OP_UNM = 28, // A<DST> = -D<VAR>
	BC_OP_ADDVN = 30, // A<DST> = B<VAR> + C<NUM>
	BC_OP_ADDVN_ALT1 = 31, // BC_ADDVN 别名
	BC_OP_ADDVN_ALT2 = 32, // BC_ADDVN 别名
	BC_OP_ADDVN_ALT3 = 33, // BC_ADDVN 别名
	BC_OP_ADDVN_ALT4 = 34, // BC_ADDVN 别名
	...
	BC_OP_UGET = 51, // A<DST> = D<UV> (本体，51-55都映射到这里)
	BC_OP_UGET_ALT1 = 52, // BC_UGET 别名
	BC_OP_UGET_ALT2 = 53, // BC_UGET 别名
	BC_OP_UGET_ALT3 = 54, // BC_UGET 别名
	BC_OP_UGET_ALT4 = 55, // BC_UGET 别名
    ...
    BC_OP_JFORI = 94, // unsupported  
	BC_OP_FORL = 95, // end of numeric for loop; start at D<JUMP>
	BC_OP_IFORL = 96, // unsupported
	BC_OP_JFORL = 97, // unsupported
	BC_OP_ITERL = 98, // end of generic for loop; start at D<JUMP>
	BC_OP_IITERL = 99, // unsupported
	BC_OP_JITERL = 100, // unsupported
	BC_OP_LOOP = 101, // if D<JUMP> == 32767 then goto loop else while/repeat loop; exit at D
	BC_OP_ILOOP = 102, // unsupported
	BC_OP_JLOOP = 103, // unsupported
	BC_OP_JMP = 104, // goto D<JUMP> or if true then JMP or goto ITERC at D
	BC_OP_FUNCF = 105, // unsupported
	BC_OP_IFUNCF = 106, // unsupported
	BC_OP_JFUNCF = 107, // unsupported
	BC_OP_FUNCC = 108, // unsupported
	BC_OP_FUNCCW = 109, // unsupported
	BC_OP_FUNCV = 110, // unsupported
	BC_OP_IFUNCV = 111, // unsupported
	BC_OP_JFUNCV = 112, // unsupported
...
static BC_OP get_op_type(const uint8_t& byte, const uint8_t& /* version */) {
	// 处理指令别名映射
	if (byte >= 12 && byte <= 20) {
		return BC_OP_ISFC;  // 12-20都映射到BC_OP_ISFC
	}
	if (byte >= 30 && byte <= 34) {
		return BC_OP_ADDVN; // 30-34都映射到BC_OP_ADDVN
	}
	if (byte >= 51 && byte <= 55) {
		return BC_OP_UGET;  // 51-55都映射到BC_OP_UGET
	}
	// 其他指令直接返回字节码值
	return (BC_OP)byte;
}
